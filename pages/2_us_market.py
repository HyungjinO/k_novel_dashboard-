...
# --- STEP 1: Add the Korean mapping dictionaries ---
...
selected_label = st.pills("독자 페르소나 필터", cluster_labels, default=cluster_labels[0])
selected_cluster_id = next((k for k, v in persona_data.items() if f"{v['emoji']} {v['name']}" == selected_label), 0)

col_persona_main, col_analysis_main = st.columns([6, 4], gap="large")

with col_persona_main:
    persona = persona_data[selected_cluster_id]
    ...
    with stylable_container("persona_details_card", css_styles=".content-card"):
        ...
        with img_col:
            img_path = f"images/cluster_{selected_cluster_id}.png"
    ...
    with stylable_container("keyword_card_bottom", css_styles=".content-card"):
        ...
        keywords = keyword_data[selected_cluster_id]
        
        # --- STEP 2: Use the Korean map to display persona keywords ---
...
with col_analysis_main:
    with stylable_container("radar_chart_card", css_styles=".content-card"):
        ...
        if not df_reviews.empty:
            cluster_df = df_reviews[df_reviews['cluster'] == selected_cluster_id]
            ...
            if not df_radar.empty:
                # --- STEP 3: Translate the 'Emotion' column before plotting ---
...
if not df_nyt.empty:
    ...
    def create_donut_chart(data_series, title_text, theme, kor_map):
        if data_series.dropna().empty: return
        counts = data_series.value_counts().reset_index()
        counts.columns = ['category', 'count']
        # Translate labels to Korean
        counts['category'] = counts['category'].map(kor_map).fillna(counts['category'])
    ...
    def create_bubble_chart(data_series, title_text, theme, kor_map):
        if data_series.dropna().empty: return
        counts = data_series.value_counts().reset_index()
        counts.columns = ['category', 'count']
        # Translate labels to Korean
    ...
    with tab_donut:
        create_donut_chart(data_series, selected_category, st.session_state.theme, config["kor"])
    with tab_treemap:
        create_treemap_chart(data_series, selected_category, config["emoji"], st.session_state.theme, config["kor"])
    with tab_bubble:
        create_bubble_chart(data_series, selected_category, st.session_state.theme, config["kor"])
else:
...
def render_stars(rating):
    ...
    html += ''.join('<span class="star full">&#9733;</span>' for _ in range(full))
    if half:
        html += '<span class="star half">&#9733;</span>'
    html += ''.join('<span class="star">&#9733;</span>' for _ in range(empty))
    html += '</span>'
    return html
